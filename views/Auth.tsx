...
const GoogleSocialButton: React.FC<{ onLogin: (user: User) => void, setError: (err: string) => void, setLoading: (l: boolean) => void }> = ({ onLogin, setError, setLoading }) => {
...
const SocialLogins: React.FC<{ onLogin: (user: User) => void, setError: (err: string) => void, setLoading: (l: boolean) => void }> = ({ onLogin, setError, setLoading }) => {
    ...
    return (
        <div className="grid grid-cols-4 gap-4">
            {isGoogleConfigured ? (
                <GoogleSocialButton onLogin={onLogin} setError={setError} setLoading={setLoading} />
            ) : (
                <button
                    disabled
                    className="flex items-center justify-center py-4 px-4 bg-white/5 border border-white/10 rounded-2xl opacity-40 cursor-not-allowed"
            ...
            <div className="w-full max-w-md bg-surface/80 border border-white/5 rounded-3xl p-10 backdrop-blur-xl shadow-2xl relative z-10">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-primary hover:bg-primary-hover disabled:opacity-50 text-black font-bold py-3 rounded-lg transition-all flex items-center justify-center gap-2 group"
                    >
